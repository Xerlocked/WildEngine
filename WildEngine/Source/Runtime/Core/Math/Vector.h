...
{
	float Temp = (Point1.X - Point2.X); if (FMath::Abs(Temp)>=Dist) return false;
...
{
	return FVector::Parallel(Normal1, Normal2) &&
		   (FVector::PointPlaneDist(Base2, Base1, Normal1) <= THRESH_POINT_ON_PLANE);
...
FORCEINLINE FVector::FVector() = default;
