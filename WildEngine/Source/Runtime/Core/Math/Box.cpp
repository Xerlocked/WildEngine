...
FBox FBox::TransformBy(const FTransform & M) const
{
	...
	{
		FVector4 ProjectedVertex = M.TransformPosition(Vertex);
...
FBox FBox::InverseTransformBy(const FTransform & M) const
{
	...
	for (auto Vertex : Vertices)
	{
		FVector4 ProjectedVertex = M.InverseTransformPosition(Vertex);
		NewBox += ProjectedVertex;
	}
...
FBox FBox::TransformProjectBy(const FMatrix& ProjM) const
{
	...
	for (auto Vertex : Vertices)
	{
		FVector4 ProjectedVertex = ProjM.TransformPosition(Vertex);
		NewBox += static_cast<FVector>(ProjectedVertex) / ProjectedVertex.W;
	}
